...
# ensuite on va pouvoir accéder à notre fils à l'aide de la fonction store_line créer dans Godot qui va nous permettre de sauvegarder nos lignes
...
# On va devoir voir si on'a un document là bas qui s'appelle ces files, si existe on va devoir faire quelque chose avec.
# donc on va créer un statement

	if file.file_exists(save_path): # on va indiquer à Godot d'aller voir dans notre project open userdata s'il y'a un document qui s'appelle savegame
